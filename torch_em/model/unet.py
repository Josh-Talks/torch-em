...
class UNetBase(nn.Module):
    ...
    def _apply_with_side_outputs(self, x):
        ...
        x, encoder_out = self.encoder(x)
        x = self.base(x)
        x, decoder_in = self.decoder(x, encoder_inputs=encoder_out[::-1])
    ...
    def get_all_features(self, x):
        ...
        x, encoder_out = self.encoder(x)
        base_x = self.base(x)
        x, decoder_in = self.decoder(base_x, encoder_inputs=encoder_out[::-1])
        # reverse list to have full shape output as first element
        return x[::-1], encoder_out[::-1], base_x, decoder_in[::-1]
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_inputs):
        ...
        for block, sampler, from_encoder in zip(
            ...
            x = sampler(x)
            decoder_in = self._concat(x, from_encoder)
            x = block(decoder_in)
        ...
        if self.return_outputs:
            return decoder_out + [x], decoder_ins
        else:
            return x
